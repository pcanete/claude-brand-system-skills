...
// Demostrar un dato no es lo mismo que citar una observación. El escáner se
// compromete en `observations`: ahí puso un modo, una confianza y su evidencia.
// El resto del documento es material que registró sin afirmarlo. Una demo puede
// apoyarse en cualquiera de los dos, pero quien aprueba el checkpoint tiene que
// poder distinguirlos sin abrir el JSON.
function sourceSupport(style, sourcePaths) {
  const observed = new Map((style.observations || []).map((item) => [item.path, item]));
  return sourcePaths.map((sourcePath) => {
    const observation = observed.get(sourcePath);
    return {
      path: sourcePath,
      observed: Boolean(observation),
      mode: observation?.mode ?? null,
      value: getPath(style, sourcePath)
    };
  });
}

function renderDemo(demo, style, evidenceIds) {
  const support = sourceSupport(style, demo.source_paths);
  const backed = support.filter((item) => item.observed);

  const badge = backed.length === support.length
    ? `<span class="support support--full">Observado · ${backed.map((item) => item.mode).join(", ")}</span>`
    : backed.length
      ? `<span class="support support--partial">${backed.length} de ${support.length} observadas</span>`
      : `<span class="support support--none">Sin observación que la respalde</span>`;

  const source = Object.fromEntries(
    support.map((item) => [
      item.observed ? `${item.path}  [observado: ${item.mode}]` : `${item.path}  [dato registrado]`,
      item.value
    ])
  );

  return `<section id="${esc(demo.id)}" class="demo" data-kind="${esc(demo.kind)}" data-support="${backed.length}/${support.length}" data-duration="${Number(demo.config.duration_ms || 1600)}"><header><div><span class="index">${esc(demo.kind)}</span><h2>${esc(demo.title)}</h2></div><div><p>${esc(demo.note)}</p>${badge}</div></header><div class="stage">${demoMarkup(demo)}</div><details><summary>Source and evidence</summary><pre>${esc(JSON.stringify({ source, evidence_refs: demo.evidence_refs.filter((ref) => evidenceIds.has(ref)) }, null, 2))}</pre></details></section>`;
}
