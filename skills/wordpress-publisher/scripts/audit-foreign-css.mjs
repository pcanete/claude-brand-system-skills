...
export function auditar(css, { scope = null } = {}) {
  ...
  for (const regla of reglas) {
    ...
    const propiedades = [...regla.cuerpo.matchAll(/(^|;)\s*([a-z-]+)\s*:/gi)].map((m) => m[2].toLowerCase());
  ...
  return {
    ...
    veredicto: peligrosas.length === 0 ? 'segura' : 'revisar'
  };
}
...
async function main() {
  ...
  for (const origen of origenes) {
    ...
    const resultado = auditar(css, { scope });
    ...
    if (!resultado.peligrosas.length) {
